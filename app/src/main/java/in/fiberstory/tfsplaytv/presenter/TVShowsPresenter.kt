...
class TVShowsPresenter(private val mContext: Activity, private val category: String) :
    ...
    override fun onBindViewHolder(card: Any, cardView: NewDocumenteryImageCardView) {
        val data = card as TVShowsDatumModel
        ...
        if (!TextUtils.isEmpty(card.showName)) {
            if (card.showName?.length!! > 14) {
                cardView.content_name?.visibility = View.VISIBLE
                cardView.content_name.text = card.showName.substring(0, 13) + "..."
            } else {
                cardView.content_name.text = card.showName
                cardView.content_name?.visibility = View.VISIBLE
            }
        } else {
